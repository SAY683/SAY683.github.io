...
class Event:
    def __init__(self, name, trigger_type, trigger_params, target_selector, effects, duration=1, one_time=False,
        ...
        self.trigger_params = trigger_params  # 必须确保每个事件定义都有此键
    ...
    def check_trigger(self, all_states_objects_dict, global_metrics, global_env_factors, current_step=0):
        ...
        # --- Conditional Global Trigger ---
            ...
            specific_source_name = self.trigger_params.get(
                'source_name_en')  # For checking a specific individual's dim globally
            ...
            if specific_source_name:
                if specific_source_name in all_states_objects_dict and \
                        all_states_objects_dict[specific_source_name] is not None and \
                        hasattr(all_states_objects_dict[specific_source_name], metric_name):
                    source_obj = all_states_objects_dict[specific_source_name]
                    metric_val = getattr(source_obj, metric_name)
                else:
                    # log_message("DEBUG", f"Event '{self.name}': Specific source '{specific_source_name}' or its dim '{metric_name}' not found.", "EventCheck")
                    return False  # Specific source or its dimension not found
            elif source_dict is not None:
                metric_val = source_dict.get(metric_name)

            if metric_val is None:
                # log_message("DEBUG", f"Event '{self.name}': Metric '{metric_name}' not found in source for conditional_global.", "EventCheck")
                return False
        ...
        # --- Conditional Individual Trigger ---
            ...
            specific_target_name = self.trigger_params.get(
                'target_name_en')  # Optional: check only one specific individual
            ...
            if specific_target_name:  # If a specific target is named for the trigger condition
                if specific_target_name in all_states_objects_dict and all_states_objects_dict[specific_target_name]:
                    state_obj = all_states_objects_dict[specific_target_name]
                    if hasattr(state_obj, dim_to_check):
                        s_val = getattr(state_obj, dim_to_check)
                        if s_val is not None:
                            if op_to_check == '>' and s_val > val_to_check: return True
                            if op_to_check == '<' and s_val < val_to_check: return True
                            if op_to_check == '==' and abs(s_val - val_to_check) < 1e-6: return True
                return False  # Specific target not found or condition not met
            else:  # Check all individuals
    ...
    def select_targets(self, all_states_objects_dict):
        ...
        target_selector_dict = {}
        if isinstance(self.target_selector, str):
            if self.target_selector == "all":  # Compatibility for old string format
                target_selector_dict = {'type': 'all'}
            else:  # Unknown string format
                log_message("ERROR",
                            f"Event '{self.name}': Unknown string value for target_selector: '{self.target_selector}'. Expected 'all' or a dict.",
                            "EventSelect")
                return []
            ...
            log_message("ERROR",
                        f"Event '{self.name}': target_selector format is invalid. Expected a dict with a 'type' key, or 'all'. Got: {self.target_selector}",
                        "EventSelect")
            return []
        ...
        elif target_selector_type == 'specific_name':  # For targeting a single specific entity
            name_en_to_target = target_selector_dict.get('name_en')
            if name_en_to_target and name_en_to_target in all_states_objects_dict and all_states_objects_dict[
                name_en_to_target]:
                targets = [all_states_objects_dict[name_en_to_target]]
            else:
                log_message("WARN", f"Event '{self.name}': Could not find specific target '{name_en_to_target}'.",
                            "EventSelect")
                targets = []
        elif target_selector_type == 'by_name_list':  # For targeting a list of specific entities
            names_en_list = target_selector_dict.get('names_en', [])
            for name_en_to_target in names_en_list:
                if name_en_to_target in all_states_objects_dict and all_states_objects_dict[name_en_to_target]:
                    targets.append(all_states_objects_dict[name_en_to_target])
                else:
                    log_message("WARN", f"Event '{self.name}': Could not find target in list '{name_en_to_target}'.",
                                "EventSelect")
        else:
            log_message("ERROR",
    ...
    def get_effects_for_target(self):
        # ... (get_effects_for_target from GM4.5.6 - no changes needed here for new selectors/triggers)
        applied_effects = []
...
initial_states_templates_gm457c = [
    # 《浩瀚无垠》角色
    {'name_zh': "阿肯(主角)", 'name_en': "AkenNovel",
     'b1_res': 4.0, 'b2_lim': 4.5,  # 初始状态，家庭负担，对现状不满
     'y1_cla': 5.0, 'y2_dri': 6.5,  # 有正义感和行动意愿，但价值观在探索
     'y3_asp': 6.0,  # 有改变世界的模糊愿望
     'h1_pos': 5.5, 'h2_inn': 4.5,  # 创新能力在事件后觉醒
     'h3_ris': 7.0,  # 从后续行为看，风险承受较高
     's1_tru': 6.5, 's2_rep': 4.5},

    {'name_zh': "文文(被阿肯救的小女孩)", 'name_en': "WenwenNovel",  # 小说中是文文，不是小女孩
     'b1_res': 1.0, 'b2_lim': 8.0,  # 极度弱势，面临直接生命危险
     'y1_cla': 4.0, 'y2_dri': 3.0,  # 惊恐，但有求生意志
     'y3_asp': 3.0,
     'h1_pos': 1.5, 'h2_inn': 1.0,
     'h3_ris': 2.0,
     's1_tru': 6.0, 's2_rep': 1.0},  # 初始无社会地位

    {'name_zh': "林上校(军方)", 'name_en': "LinColonelNovel",
     'b1_res': 7.5, 'b2_lim': 2.0,
     'y1_cla': 7.0, 'y2_dri': 7.5,  # 军人职责感
     'y3_asp': 6.0,
     'h1_pos': 6.0, 'h2_inn': 5.5,
     'h3_ris': 6.0,
     's1_tru': 7.0, 's2_rep': 8.0},

    {'name_zh': "王上将(军方高层)", 'name_en': "WangGeneralNovel",
     'b1_res': 8.5, 'b2_lim': 1.0,
     'y1_cla': 8.0, 'y2_dri': 6.5,  # 更注重宏观和稳定
     'y3_asp': 7.5,
     'h1_pos': 6.5, 'h2_inn': 6.5,
     'h3_ris': 3.5,  # 位高者通常更厌恶不可控风险
     's1_tru': 6.5, 's2_rep': 9.2},

    {'name_zh': "小飞(阿肯好友)", 'name_en': "XiaoFeiNovel",
     'b1_res': 3.5, 'b2_lim': 5.0,
     'y1_cla': 6.0, 'y2_dri': 7.0,  # 讲义气，有行动力
     'y3_asp': 4.5,
     'h1_pos': 5.0, 'h2_inn': 4.0,
     'h3_ris': 6.5,
     's1_tru': 7.0, 's2_rep': 4.0},

    {'name_zh': "安妮(遗弃者联盟)", 'name_en': "AnnieNovel",  # 假设为小说中的安妮
     'b1_res': 4.5, 'b2_lim': 3.0,
     'y1_cla': 7.8, 'y2_dri': 6.5,  # 理性，有计划
     'y3_asp': 6.8,
     'h1_pos': 6.5, 'h2_inn': 7.0,
     'h3_ris': 5.5,
     's1_tru': 7.0, 's2_rep': 6.0},

    {'name_zh': "克里斯(遗弃者联盟)", 'name_en': "ChrisNovel",
     'b1_res': 4.0, 'b2_lim': 3.5,
     'y1_cla': 5.5, 'y2_dri': 6.8,
     'y3_asp': 5.0,
     'h1_pos': 5.2, 'h2_inn': 5.2,
     'h3_ris': 6.2,
     's1_tru': 6.8, 's2_rep': 4.8},

    {'name_zh': "布林肯(裁判所)", 'name_en': "BlinkenNovel",
     'b1_res': 7.0, 'b2_lim': 2.5,
     'y1_cla': 8.5, 'y2_dri': 8.0,  # 对其“真理”有极高清晰度和驱动力
     'y3_asp': 9.0,  # 认为自己在执行崇高使命
     'h1_pos': 6.0, 'h2_inn': 7.0,  # 可能有特殊的技术或手段
     'h3_ris': 7.0,  # 为了“真理”不惜一切
     's1_tru': 3.0, 's2_rep': 8.5},  # 在其体系内声望高，但对外界可信度低

    {'name_zh': "龙哥(工厂工友)", 'name_en': "LongGeNovel",
     'b1_res': 2.5, 'b2_lim': 6.5,  # 生活困苦
     'y1_cla': 5.0, 'y2_dri': 4.0,  # 被生活所迫，驱动力不高
     'y3_asp': 3.0,
     'h1_pos': 2.0, 'h2_inn': 1.5,
     'h3_ris': 2.5,
     's1_tru': 6.0, 's2_rep': 3.0},

    {'name_zh': "夏目德人(黑帮/商人)", 'name_en': "NatsumeTokujinNovel",  # 小说中的夏目德人
     'b1_res': 7.0, 'b2_lim': 3.0,  # 掌握一定资源
     'y1_cla': 6.0, 'y2_dri': 7.5,  # 目标是利益
     'y3_asp': 5.5,
     'h1_pos': 6.5, 'h2_inn': 5.0,  # 可能会用一些手段
     'h3_ris': 7.0,  # 逐利者通常有风险偏好
     's1_tru': 2.5, 's2_rep': 6.5},  # 声望可能基于势力而非美德
    # 例如，可以细化阿肯的初始值，使其更符合小说开篇
    {'name_zh': "阿肯(初期)", 'name_en': "AkenInitialNovel",
     'b1_res': 3.0, 'b2_lim': 5.5,  # 家庭经济压力，对现实的无力感
     'y1_cla': 3.5, 'y2_dri': 5.0,  # 考试刚完，目标尚不明确，但有朴素的正义感
     'y3_asp': 4.5,  # 对未来有朦胧的期望
     'h1_pos': 4.0, 'h2_inn': 3.0,  # 尚未发掘潜力
     'h3_ris': 5.5,  # 有一定的冲动和冒险精神
     's1_tru': 6.0, 's2_rep': 3.5},

    {'name_zh': "莎姐(军方/监察)", 'name_en': "SarahNovel",  # 对应小说中的莎拉/莎姐
     'b1_res': 5.5, 'b2_lim': 3.0,
     'y1_cla': 7.0, 'y2_dri': 7.0,  # 任务导向，有正义感
     'y3_asp': 6.0,
     'h1_pos': 5.0, 'h2_inn': 5.5,  # 具备一定技能
     'h3_ris': 6.0,
     's1_tru': 6.5, 's2_rep': 6.0},

    {'name_zh': "梅姨(王长官老伴)", 'name_en': "MeiYiNovel",
     'b1_res': 5.0, 'b2_lim': 2.5,
     'y1_cla': 7.0, 'y2_dri': 4.0,  # 温和，关怀
     'y3_asp': 4.0,
     'h1_pos': 3.0, 'h2_inn': 2.0,
     'h3_ris': 2.0,
     's1_tru': 8.0, 's2_rep': 6.5},

    {'name_zh': "斯尔特(赏金猎人)", 'name_en': "SulterNovel",
     'b1_res': 7.0, 'b2_lim': 1.5,  # 装备精良，能力强
     'y1_cla': 4.0, 'y2_dri': 8.0,  # 目标驱动（可能是金钱或命令）
     'y3_asp': 5.0,
     'h1_pos': 6.0, 'h2_inn': 7.0,  # 战斗技巧高
     'h3_ris': 8.5,  # 高风险职业
     's1_tru': 2.0, 's2_rep': 7.0},  # 可能是恶名昭彰

    # 可以为仓库工人等群体设定一个或多个代表性的“原型”
    {'name_zh': "仓库工人代表", 'name_en': "WarehouseWorkerRep",
     'b1_res': 1.5, 'b2_lim': 7.5,  # 资源匮乏，限制多
     'y1_cla': 3.0, 'y2_dri': 2.5,  # 长期压迫可能导致价值模糊，驱动力低
     'y3_asp': 2.0,  # 理想被磨灭
     'h1_pos': 1.5, 'h2_inn': 1.0,
     'h3_ris': 4.0,  # 被逼急了也可能反抗
     's1_tru': 5.0, 's2_rep': 2.0},
]
neighbor_config_gm457c = {
    "AkenNovel": ["WenwenNovel", "XiaoFeiNovel", "AnnieNovel", "ChrisNovel", "LinColonelNovel", "WangGeneralNovel",
                  "BlinkenNovel", "LongGeNovel", "NatsumeTokujinNovel"],
    "WenwenNovel": ["AkenNovel"],
    "LinColonelNovel": ["AkenNovel", "WangGeneralNovel"],
    "WangGeneralNovel": ["AkenNovel", "LinColonelNovel"],
    "XiaoFeiNovel": ["AkenNovel", "AnnieNovel", "ChrisNovel", "WenwenNovel", "LongGeNovel"],
    "AnnieNovel": ["AkenNovel", "XiaoFeiNovel", "ChrisNovel", "WenwenNovel"],
    "ChrisNovel": ["AkenNovel", "XiaoFeiNovel", "AnnieNovel", "WenwenNovel"],
    "BlinkenNovel": ["AkenNovel"],  # 主要对手戏
    "LongGeNovel": ["XiaoFeiNovel", "AkenNovel"],  # 工友关系
    "NatsumeTokujinNovel": ["AkenNovel"],  # 发生过冲突
}
neighbor_config_gm457c.update({
    "AkenInitialNovel": ["WenwenNovel", "XiaoFeiNovel", "AnnieNovel", "ChrisNovel", "LinColonelNovel",
                         "WangGeneralNovel", "BlinkenNovel", "LongGeNovel", "NatsumeTokujinNovel", "SarahNovel"],
    "SarahNovel": ["AkenInitialNovel", "LinColonelNovel", "WangGeneralNovel"],  # 与军方和阿肯有联系
    "WangGeneralNovel": ["AkenInitialNovel", "LinColonelNovel", "MeiYiNovel", "SulterNovel"],  # 与梅姨、斯尔特有关联
    "MeiYiNovel": ["WangGeneralNovel", "AkenInitialNovel", "XiaoFeiNovel"],  # 可能与年轻人有交集
    "SulterNovel": ["WangGeneralNovel"],  # 主要听命于王上将
    # ... 其他角色的邻居关系也需要考虑这些新加入的角色 ...
})
event_definitions_gm457c = [
    # --- 小说核心灾难事件 ---
    {'name': "“慌乱”-怪兽首次低空掠过(阿肯家)", 'trigger_type': "timed",
     'trigger_params': {'step': 5},  # 模拟初期发生
     'target_selector': {'type': 'specific_name', 'name_en': 'AkenNovel'},  # 主要影响阿肯的感知
     'effects': [{'dim': 'mood', 'type': 'add_abs', 'val': -0.5, 'rand_range': 0.1},  # 造成恐慌
                 {'dim': 'b2_limitation', 'type': 'add_abs', 'val': 1.0, 'rand_range': 0.2}],  # 周围环境的初始混乱
     'duration': 1, 'one_time': True,
     'chain_event_name': "电视紧急避难通知(小说)", 'chain_event_delay': 0, 'chain_event_prob': 1.0},

    {'name': "电视紧急避难通知(小说)", 'trigger_type': "none", 'trigger_params': {},
     'target_selector': "all",  # 全局通知
     'effects': [{'dim': 'mood', 'type': 'add_abs', 'val': -0.2, 'rand_range': 0.05}],  # 进一步加剧紧张气氛
     'one_time': True,
     'chain_event_name': "怪兽正式来袭-天空黑影(小说)", 'chain_event_delay': 2, 'chain_event_prob': 1.0},

    {'name': "怪兽正式来袭-天空黑影(小说)", 'trigger_type': "none", 'trigger_params': {},
     'target_selector': "all",  # 全局性灾难
     'effects': [{'dim': 'b1_resource', 'type': 'multiply_abs', 'val': 0.7, 'rand_range': 0.1},  # 资源普遍受损
                 {'dim': 'b2_limitation', 'type': 'add_abs', 'val': 2.5, 'rand_range': 0.4},  # 大量设施被毁
                 {'dim': 'h1_possibilities', 'type': 'multiply_abs', 'val': 0.6, 'rand_range': 0.1},  # 未来充满不确定
                 {'dim': 'mood', 'type': 'add_abs', 'val': -0.7, 'rand_range': 0.1}],  # 深度绝望
     'duration': 8, 'chain_event_name': "军队初步抵抗(小说)", 'chain_event_delay': 1, 'chain_event_prob': 0.9},

    {'name': "军队初步抵抗(小说)", 'trigger_type': "none", 'trigger_params': {},
     'target_selector': "all",  # 主要影响军方角色，但氛围影响全局
     'effects': [{'dim': 'y2_drive', 'type': 'add_abs', 'val': 0.5,
                  'target_filter_names': ['LinColonelNovel', 'WangGeneralNovel']},  # 军人驱动力提升
                 {'dim': 'mood', 'type': 'add_abs', 'val': 0.1, 'rand_range': 0.05}],  # 一丝希望或紧张
     'duration': 5, 'one_time': True},  # 抵抗有时间限制

    # --- 阿肯的转变 ---
    {'name': "阿肯发现鹰角(变身器)", 'trigger_type': "conditional_individual",  # 在阿肯经历危机后
     'trigger_params': {'dim': 'b1_resource', 'op': '<', 'val': 1.5, 'target_name_en': 'AkenNovel'},  # 阿肯资源极低且绝望时
     'target_selector': {'type': 'specific_name', 'name_en': 'AkenNovel'},
     'effects': [{'dim': 'h2_innovation', 'type': 'set_abs', 'val': 7.0},  # 突然获得巨大潜能
                 {'dim': 'h3_risk_appetite', 'type': 'set_abs', 'val': 8.0},
                 {'dim': 'y3_aspiration', 'type': 'add_abs', 'val': 3.0},  # 理想被激发
                 {'dim': 'mood', 'type': 'set_abs', 'val': 0.2}],  # 从绝望中看到一丝曙光
     'one_time': True, 'chain_event_name': "阿肯首次变身为高斯(小说)", 'chain_event_delay': 0, 'chain_event_prob': 1.0},

    {'name': "阿肯首次变身为高斯(小说)", 'trigger_type': "none", 'trigger_params': {},
     'target_selector': {'type': 'specific_name', 'name_en': 'AkenNovel'},
     'effects': [{'dim': 's2_reputation', 'type': 'add_abs', 'val': 3.0, 'rand_range': 0.5},  # 巨大声望提升
                 {'dim': 'y2_drive', 'type': 'set_abs', 'val': 8.5},  # 驱动力爆发
                 {'dim': 'b1_resource', 'type': 'add_abs', 'val': -1.0, 'rand_range': 0.3},  # 变身消耗
                 {'dim': 'b2_limitation', 'type': 'set_abs', 'val': 1.0}],  # 变身后限制大幅降低
     'duration': 10, 'one_time': True},  # 奥特曼有时间限制，这里用duration模拟

    # --- 小说中的其他冲突和互动 ---
    {'name': "仓库遭遇裁判所(布林肯)", 'trigger_type': "conditional_individual",
     'trigger_params': {'dim': 's2_reputation', 'op': '>', 'val': 6.0, 'target_name_en': 'AkenNovel'},  # 阿肯有一定名气后
     'target_selector': {'type': 'specific_name', 'name_en': 'AkenNovel'},  # 或针对一个区域内的多个角色
     'effects': [{'dim': 'b2_limitation', 'type': 'add_abs', 'val': 1.5, 'rand_range': 0.3},  # 遭遇阻碍
                 {'dim': 's1_trustworthiness', 'type': 'add_abs', 'val': -1.0, 'target_filter_names': ['BlinkenNovel']},
                 # 布林肯可信度在阿肯眼中下降
                 {'dim': 'mood', 'type': 'add_abs', 'val': -0.3}],
     'duration': 3, 'one_time': True},

    # 保留一些通用事件，可以根据小说背景调整其描述或效果
    {'name': "社会信任波动", 'trigger_type': "probabilistic", 'trigger_params': {'prob': 0.03},
     'target_selector': "all",
     'effects': [{'dim': 's1_trustworthiness', 'type': 'add_abs', 'val': random.uniform(-0.5, 0.3), 'rand_range': 0.2},
                 {'dim': 'mood', 'type': 'add_abs', 'val': random.uniform(-0.1, 0.1)}],
     'duration': 2},
    # --- 小说核心战斗/转折事件 ---
    {'name': "商场屠夫怪人事件(小说)", 'trigger_type': "timed",
     'trigger_params': {'step': 30},
     ### 将下面的类型改为我们已实现的类型 ###
     'target_selector': {'type': 'random_n_neighbors',  # <--- 修改这里
                         'n_clusters': 1,  # 选择一个中心点（比如阿肯）
                         'center_agent_name_en': 'AkenInitialNovel',  # 新增参数指定中心点
                         'cluster_size_avg': 2,  # 选择中心点 + 1个附近的倒霉蛋
                         'cluster_size_std': 0,
                         'exclude_center': True},  # 不包括中心点（阿肯）本身作为目标
     'effects': [{'dim': 'b1_resource', 'type': 'add_abs', 'val': -5.0, 'rand_range': 0.2, 'on_target': True},
                 # 这个 on_target 不是标准效果参数
                 {'dim': 'mood', 'type': 'add_abs', 'val': -0.5}],  # 假设对选中的目标施加
     'duration': 1, 'one_time': True,
     'chain_event_name': "阿肯救助文文(小说)", 'chain_event_delay': 0, 'chain_event_prob': 0.9},

    {'name': "阿肯救助文文(小说)", 'trigger_type': "none", 'trigger_params': {},
     'target_selector': {'type': 'specific_name', 'name_en': 'AkenInitialNovel'},
     'effects': [{'dim': 'y2_drive', 'type': 'add_abs', 'val': 1.0},  # 激发阿肯的驱动力
                 {'dim': 's1_trustworthiness', 'type': 'add_abs', 'val': 1.0,
                  'target_state_name_for_trust_change': 'WenwenNovel'}],  # 文文对阿肯信任增加，这个效果类型需要实现
     'one_time': True},

    {'name': "银行遭遇裁判所(小说)", 'trigger_type': "conditional_global",  # 例如，在阿肯S2达到一定值后，或某个区域发生特定事件
     'trigger_params': {'dim': 'avg_s2_reputation', 'op': '>', 'val': 5.0, 'source': 'metrics'},  # 粗略模拟
     'target_selector': {'type': 'by_name_list', 'names_en': ['AkenInitialNovel', 'SarahNovel', 'BlinkenNovel']},
     'effects': [{'dim': 'b2_limitation', 'type': 'add_abs', 'val': 2.0, 'rand_range': 0.3},  # 遭遇强大敌人
                 {'dim': 'mood', 'type': 'add_abs', 'val': -0.4}],
     'duration': 3, 'one_time': True, 'chain_event_name': "莎姐的牺牲(小说)", 'chain_event_delay': 2,
     'chain_event_prob': 0.6},

    {'name': "莎姐的牺牲(小说)", 'trigger_type': "none", 'trigger_params': {},
     'target_selector': {'type': 'specific_name', 'name_en': 'SarahNovel'},  # 莎姐状态改变
     'effects': [{'dim': 'b1_resource', 'type': 'set_abs', 'val': 0.01},  # 濒死
                 {'dim': 'is_active_participant', 'type': 'set_bool', 'val': False}],  # 从活跃状态移除，这个效果类型需要实现
     'one_time': True, 'chain_event_name': "阿肯的愤怒(小说)", 'chain_event_delay': 0, 'chain_event_prob': 1.0},

    {'name': "阿肯的愤怒(小说)", 'trigger_type': "none", 'trigger_params': {},
     'target_selector': {'type': 'specific_name', 'name_en': 'AkenInitialNovel'},
     'effects': [{'dim': 'y2_drive', 'type': 'set_abs', 'val': 9.5},  # 驱动力爆发
                 {'dim': 'h3_risk_appetite', 'type': 'set_abs', 'val': 9.0},  # 变得极度冒险
                 {'dim': 'mood', 'type': 'set_abs', 'val': -0.9},  # 极度负面情绪
                 {'dim': 'y1_clarity', 'type': 'add_abs', 'val': -2.0}],  # 价值观可能受到冲击
     'duration': 5, 'one_time': True},

    {'name': "生命之树的真相(小说)", 'trigger_type': "conditional_global",  # 可能在游戏后期，王长官S2极高时
     'trigger_params': {'dim': 's2_reputation', 'op': '>', 'val': 8.5, 'source_name_en': 'WangGeneralNovel'},
     # 需要检查特定人的维度
     'target_selector': "all",
     'effects': [{'dim': 'y1_clarity', 'type': 'add_abs', 'val': -1.0, 'rand_range': 0.5},  # 普遍的价值观冲击
                 {'dim': 'mood', 'type': 'add_abs', 'val': -0.2, 'rand_range': 0.1}],
     'one_time': True},

    {'name': "最终决战-日蚀形态(小说)", 'trigger_type': "conditional_individual",  # 阿肯在特定条件下（如濒死反击）
     'trigger_params': {'dim': 'b1_resource', 'op': '<', 'val': 0.5, 'target_name_en': 'AkenInitialNovel',
                        'and_event_active': '生命之树的真相(小说)'},  # 假设需要前置事件
     'target_selector': {'type': 'specific_name', 'name_en': 'AkenInitialNovel'},
     'effects': [{'dim': 'b1_resource', 'type': 'set_abs', 'val': 8.0},  # 力量大幅提升
                 {'dim': 'h2_innovation', 'type': 'set_abs', 'val': 9.8},
                 {'dim': 'y2_drive', 'type': 'set_abs', 'val': 9.8},
                 {'dim': 's1_trustworthiness', 'type': 'set_abs', 'val': 9.0},  # 成为救世主，信任度爆表
                 {'dim': 's2_reputation', 'type': 'set_abs', 'val': 9.8}],
     'one_time': True, 'duration': 10},  # 强大的形态通常有时间限制

    # ... 其他小说中具体的小冲突、民众反抗等可以设计成概率性或条件性事件 ...
]
...
# Update with GM4.5.7a (Numerical Extremes) specific general params
...
# Update with GM4.5.7b (Cognitive Deepening) specific general params
default_evolution_params_gm457c.update({
    ...
    ### ADDED for GM4.5.7c Right Panel Enhancement ###
    'max_timeseries_points': 200,  # 新增：时间序列图最大数据点
    ### END ADDED ###
})
...
app.layout = html.Div([
    ...
    dcc.Store(id='evolution-params-store', data=default_evolution_params_gm457c),  # 使用更新后的参数
    ...
    ### ADDED: Store for timeseries data ###
    ...
    html.Div([
        html.Div([  # 左侧面板
            ...
            ### ADDED: 3D Plot Controls ###
            html.H4("3D图表控制", style={'marginTop': '10px', 'borderTop': '1px solid #ccc', 'paddingTop': '8px'}),
            dcc.Checklist(
                id='history-trajectory-checklist',
                options=[
                    {'label': '显示选中对象历史轨迹', 'value': 'SELECTED'},
                    {'label': '显示所有历史轨迹', 'value': 'ALL'},
                ],
                value=['SELECTED'],  # Default
                labelStyle={'display': 'block', 'fontSize': 'small'},
                style={'marginBottom': '5px'}
            ),
            html.Button('清空所有历史轨迹', id='clear-all-history-button', n_clicks=0,
                        style={'fontSize': 'small', 'width': '100%', 'marginBottom': '10px'}),
            ### END ADDED ###
            ...
            html.Details([html.Summary("显示/隐藏参数控件", style={'cursor': 'pointer', 'marginBottom': '5px'}),
                          # ... (Existing param controls from GM4.5.7c) ...
        ...
        ### MODIFIED: Right Panel Structure ###
        html.Div([
            dcc.Graph(id='main-3d-scatter-plot', style={'height': '50vh'}),  # Adjusted height
            html.Div(id='timeseries-plot-div', children=[  # Timeseries plot
                dcc.Graph(id='selected-state-timeseries-plot', style={'height': '20vh'})  # Height for timeseries
            ], style={'marginTop': '5px', 'borderTop': '1px solid #ddd', 'paddingTop': '5px'}),
            html.Div([  # Community Projects and Log
                html.Div([
                    html.H5("社区项目:", style={'marginTop': '5px', 'marginBottom': '2px'}),
                    html.Div(id='community-projects-display-div',
                             style={'width': '100%', 'height': '8vh', 'fontSize': 'x-small', 'border': '1px solid #afa',
                                    'backgroundColor': '#f0fff0', 'overflowY': 'auto', 'padding': '5px'})
                ], style={'width': '49%', 'display': 'inline-block', 'verticalAlign': 'top'}),
                html.Div([
                    html.H5("模拟日志 (最新 %s 条):" % MAX_LOG_LINES,
                            style={'marginTop': '5px', 'marginBottom': '2px'}),
                    dcc.Textarea(id='simulation-log-textarea', value="", readOnly=True,
                                 style={'width': '100%', 'height': '8vh', 'fontSize': 'x-small',
                                        'border': '1px solid #ddd', 'backgroundColor': '#fafafa',
                                        'fontFamily': 'monospace'})
                ], style={'width': '49%', 'display': 'inline-block', 'marginLeft': '2%', 'verticalAlign': 'top'})
            ], style={'height': '12vh'})  # Combined height for projects and log
        ], style={'width': '70%', 'float': 'right', 'padding': '10px', 'boxSizing': 'border-box'})
        ### END MODIFIED ###
    ]),
    html.Div(style={'clear': 'both'}),
    html.Footer(f"三世界理论模型 - v4.5.7c (UI增强) - {time.strftime('%Y-%m-%d %H:%M:%S')}",
...
def reset_all_states_gm457c(n_clicks):
    ...
    for obj in temp_list:
        obj.neighbors = neighbor_config_gm457c.get(obj.name_en, []);
        obj.clear_history();
...
@app.callback(
    [Output('world-states-store', 'data'),
     ...
     Output('community-projects-store', 'data'),
     Output('timeseries-data-store', 'data')],  # ADDED Output for timeseries
    ...
    [State('world-states-store', 'data'), State('evolution-params-store', 'data'),
     ...
     State('timeseries-data-store', 'data')]  # ADDED Input state for timeseries
)
def run_evolution_step_gm457c(n_auto_interval, n_manual_click, states_json, evo_params_json, interval_disabled,
                              ...
                              community_projects_json_data, existing_timeseries_data):  # Added existing_timeseries_data
    ctx = callback_context;
    ...
    if not all([states_json, evo_params_json, env_json]): log_message("WARN", "Evo step: missing data.",
                                                                      "演化循环");raise PreventUpdate

    current_sim_step_for_logic = 0;
    display_step_type = ""
    if triggered_id == 'evolution-interval':
        current_sim_step_for_logic = n_auto_interval;display_step_type = "自动"
    elif triggered_id == 'step-evolution-button':
        current_sim_step_for_logic = (
                                                 interval_current_n_intervals or 0) + n_manual_click;display_step_type = f"手动 ({n_manual_click})"
    ...
    metrics = {'avg_b1_resource': np.mean(
        [s.b1_resource for s in valid_states_for_metrics]) if valid_states_for_metrics else 0,
               ...
               'avg_y1_diversity': np.std([s.y1_clarity for s in valid_states_for_metrics]) if len(
                   valid_states_for_metrics) > 1 else 0.0}  # Example derived metric
    ...
    # --- Initialize or update timeseries data ---
    new_timeseries_data = existing_timeseries_data.copy() if existing_timeseries_data else {}
    dims_to_track_ts = ['b1_resource', 'y2_drive', 'h2_innovation', 's2_reputation', 'mood']  # Dims for timeseries
    ...
    for name, obj in obj_dict.items():
        ...
        try:
            ...
            # Append to timeseries data for this state
            if name not in new_timeseries_data:
                new_timeseries_data[name] = {dim: [] for dim in dims_to_track_ts}
                new_timeseries_data[name]['step'] = []

            new_timeseries_data[name]['step'].append(current_sim_step_for_logic)
            for dim_ts in dims_to_track_ts:
                new_timeseries_data[name][dim_ts].append(getattr(obj, dim_ts, None))  # Use None if attr missing

            # Limit timeseries length
            max_ts_points = evo_params_json.get('max_timeseries_points', 200)
            if len(new_timeseries_data[name]['step']) > max_ts_points:
                new_timeseries_data[name]['step'] = new_timeseries_data[name]['step'][-max_ts_points:]
                for dim_ts in dims_to_track_ts:
                    new_timeseries_data[name][dim_ts] = new_timeseries_data[name][dim_ts][-max_ts_points:]
    ...
    return updated_json_out, step_info, final_log_for_ui, community_projects_to_store, new_timeseries_data


### ADDED: Callback for 3D plot click -> update point selector ###
@app.callback(
    Output('point-selector-dropdown', 'value', allow_duplicate=True),
    Input('main-3d-scatter-plot', 'clickData'),
    State('point-selector-dropdown', 'options'),  # To ensure the ID is valid
    prevent_initial_call=True
)
def update_selector_on_plot_click_gm457c(clickData, existing_options):
    ...
    if 'customdata' in clicked_point_data and isinstance(clicked_point_data['customdata'], list) and len(
            clicked_point_data['customdata']) > 0:
        selected_id = clicked_point_data['customdata'][0]

    if selected_id:
        valid_ids = [opt['value'] for opt in existing_options]
        if selected_id in valid_ids:
            return selected_id
    raise PreventUpdate


### ADDED: Callback for "Clear All History" button ###
@app.callback(
    Output('world-states-store', 'data', allow_duplicate=True),  # Trigger a re-render of the plot
    Input('clear-all-history-button', 'n_clicks'),
    State('world-states-store', 'data'),
    prevent_initial_call=True
)
def clear_all_histories_gm457c(n_clicks, states_data_json):
    if n_clicks == 0 or not states_data_json:
        raise PreventUpdate

    updated_states = {}
    for state_id, state_data in states_data_json.items():
        new_state_data = state_data.copy()
        new_state_data['history'] = []
        updated_states[state_id] = new_state_data

    log_message("INFO", "所有状态点的历史轨迹已清空。", "UI控制")
    return updated_states  # This will trigger update_3d_scatter_plot_gm457c


### ADDED: Callback for Timeseries Plot ###
@app.callback(
    Output('selected-state-timeseries-plot', 'figure'),
    [Input('selected-point-id-store', 'data'),
     Input('timeseries-data-store', 'data'),
     Input('world-states-store', 'data')]  # To get name_zh for title
)
def update_timeseries_plot_gm457c(selected_id, timeseries_data, states_data_json):  # Renamed to match usage
    if not selected_id or not timeseries_data or selected_id not in timeseries_data:
        fig = go.Figure(layout={'title': '选择一个状态点以查看其维度历史', 'height': 200, 'xaxis': {'visible': False},
                                'yaxis': {'visible': False}})
    ...
    # 安全地获取状态中文名
    state_name_zh = "未知状态"
    if states_data_json and selected_id in states_data_json and states_data_json[selected_id]:
        state_name_zh = states_data_json[selected_id].get('name_zh', selected_id)
    elif selected_id:  # Fallback if state_data is missing but we have an ID
        state_name_zh = selected_id

    state_ts_data = timeseries_data[selected_id]
    fig = go.Figure()

    dims_to_plot_ts = ['b1_resource', 'y2_drive', 'h2_innovation', 's2_reputation', 'mood']
    # 使用Plotly默认颜色循环通常更好，但如果需要固定颜色，可以定义
    # colors = ['blue', 'green', 'red', 'purple', 'orange']

    if not state_ts_data.get('step') or not any(state_ts_data.get(dim) for dim in dims_to_plot_ts):
        fig = go.Figure(
            layout={'title': f'{state_name_zh} 尚无时间序列数据', 'height': 200, 'xaxis': {'visible': False},
                    'yaxis': {'visible': False}})
        fig.add_annotation(text="无数据", xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
        return fig

    for idx, dim_key in enumerate(dims_to_plot_ts):
        if dim_key in state_ts_data and state_ts_data[dim_key] and \
                state_ts_data['step'] and len(state_ts_data['step']) == len(
            state_ts_data[dim_key]):  # Check length consistency

            # --- 获取图例名称并汉化 'mood' ---
            base_name = DIMENSION_LABELS_MAP_ZH.get(dim_key, dim_key)  # Get full name or key itself
            if ':' in base_name:
                trace_name = base_name.split(': ')[-1].split(' ')[0]  # "资源/条件" -> "资源"
            else:  # Handles keys not in map, like 'mood' initially
                trace_name = base_name

            if dim_key == 'mood':
                trace_name = "情绪状态"  # 明确汉化
            elif dim_key in DIMENSION_LABELS_MAP_ZH:  # 对于在DIM_KEYS中的其他维度，可以进一步简化名称
                # 例如，从 "本然 B1: 资源/条件" 中提取 "资源/条件" 或 "资源"
                parts = DIMENSION_LABELS_MAP_ZH[dim_key].split(': ')
                if len(parts) > 1:
                    trace_name = parts[1].split(' ')[0]  # "资源/条件" -> "资源"
                else:
                    trace_name = parts[0]  # 如果没有冒号，就用全名
            # --- 结束图例名称处理 ---

            fig.add_trace(go.Scatter(
                x=state_ts_data['step'],
                y=state_ts_data[dim_key],
                mode='lines+markers',
                name=trace_name,
                marker=dict(size=3),
                line=dict(width=1.2)  # Let Plotly handle colors by default, or use your 'colors' list
                # line=dict(width=1.2, color=colors[idx % len(colors)]) # If using custom colors list
            ))

    fig.update_layout(
        title_text=f"{state_name_zh} 关键维度历史",
        title_x=0.5, title_font_size=14,
        xaxis_title="模拟步骤", yaxis_title="维度值 (0-10)",
        margin=dict(l=40, r=10, t=30, b=30),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, font_size=9),
        height=200,
        hovermode="x unified",
        xaxis=dict(showgrid=True, zeroline=False),
        yaxis=dict(range=[-0.5, 10.5], showgrid=True, zeroline=False, nticks=6)
    )
    return fig


### MODIFIED: update_3d_scatter_plot_gm457c to accept new inputs for history ###
@app.callback(
    Output('main-3d-scatter-plot', 'figure'),
    [Input('world-states-store', 'data'),
     Input('coord-type-dropdown', 'value'),
     Input('history-trajectory-checklist', 'value'),
     Input('selected-point-id-store', 'data')],
    [State('evolution-params-store', 'data')]
)
def update_3d_scatter_plot_gm457c(states_data_json, coord_type, history_display_options, selected_point_id_for_history,
                                  evo_params_json):
    ...
    for i, (state_id, state_dict) in enumerate(state_items):
        ...
        if has_allies and has_rivals:
            marker_symbol_str = 'x';marker_line_color = 'purple';marker_line_width = 3
        elif has_allies:
            marker_symbol_str = 'diamond';marker_line_color = 'green';marker_line_width = 2
        elif has_rivals:
            marker_symbol_str = 'cross';marker_line_color = 'red';marker_line_width = 2
        ...
        try:
            ...
            log_message("CRITICAL",
                        ...
                        "绘图")
            # Fallback plotting remains

        show_history_for_this_state = False
        if 'ALL' in history_display_options:
            show_history_for_this_state = True
        elif 'SELECTED' in history_display_options and selected_point_id_for_history == state_id:
            show_history_for_this_state = True

        if show_history_for_this_state and state_obj.history and len(
                state_obj.history) > 1 and not state_obj.is_bankrupt:
    ...
    fig.update_layout(margin=dict(l=0, r=0, b=0, t=40), title=dict(
        text=f"三世界坐标 ({'Simp.' if coord_type == 'simplified' else 'Comp.'}) - v4.5.7c (UI增强)", x=0.5, y=0.98,
        font=dict(size=14)),
...
if __name__ == '__main__':
    app.run(debug=True, port=8063)  # 使用新端口
